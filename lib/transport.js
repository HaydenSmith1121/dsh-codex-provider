 ...
 *    from `chatgpt.com/backend-api/codex`, which speaks the Responses API with
 *    additional required headers (`chatgpt-account-id`, `originator`,
...
/** Client version reported to the backend, which validates it on some routes. */
export const CLIENT_VERSION = '0.154.0'
 ...
 * @returns a connected socket, TLS-wrapped for https.
 */
async function connect({ host, port, secure, proxy, timeoutMs, signal }) {
  const onAbort = () => socket?.destroy(new Error('request aborted'))
  let socket
  const guard = () => {
...
export async function codexRequest(options) {
  ...
  const socket = await connect({ host, port, secure, proxy, timeoutMs, signal })
