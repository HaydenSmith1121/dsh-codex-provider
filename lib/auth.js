...
/**
 * Discover the Codex client version from the local installation.
 *
 * The backend validates `client_version` on the catalog route, so reporting a
 * version the backend considers stale is a real failure mode. Reading the
 * version the installed CLI actually uses keeps the plugin in step with it
 * across Codex upgrades, instead of pinning a literal that silently ages out.
 *
 * Three sources are tried, cheapest and most authoritative first:
 *   1. `models_cache.json`, which records the version the CLI last fetched with;
 *   2. the installed `@openai/codex` package manifest;
 *   3. nothing — the caller falls back to its own default.
 *
 * @param codexHome - the Codex home directory.
 * @returns the discovered version, or undefined when none is readable.
 */
export async function discoverClientVersion(codexHome) {
  const candidates = [
    async () => {
      const cache = JSON.parse(await readFile(join(codexHome, 'models_cache.json'), 'utf8'))
      return typeof cache?.client_version === 'string' ? cache.client_version : undefined
    },
    async () => {
      // The npm wrapper package version tracks the binary it downloads.
      const manifest = JSON.parse(
        await readFile(join(dirname(process.execPath), 'node_modules', '@openai', 'codex', 'package.json'), 'utf8'),
      )
  ...
  for (const attempt of candidates) {
    try {
      ...
      // Only a plain dotted release is usable; an alpha tag is not what the
      // backend's own client reports.
